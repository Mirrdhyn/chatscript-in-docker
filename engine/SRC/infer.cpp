...
static MEANING scanQueue[MAX_QUEUE+20]; 
...
static bool SetContains1(MEANING set,MEANING M, unsigned int depth) 
{
	...
	{
		SetFactBack(D,0); 
		if (trace & TRACE_INFER && CheckTopicTrace("^query")) Log(USERLOG," %s ? %s ",D->word,D1->word);
	}
	...
	while (F && --counter)
	{
		...
		else if (F->verb == Mmember)
		{
			...
			if (F->object == set && !blocked) 
			{
				...
				{
					Log(USERLOG,"\r\n");
					Log(USERLOG,"within: %s ",D1->word);
					WORDP path = Meaning2Word(F->subject); 
					while (path)
					{
						Log(USERLOG," %s ",path->word);
...
FunctionResult QueryTopicsOf(char* word,unsigned int store,char* kind) // find topics referred to by word
{
    ...
    if (trace & TRACE_INFER) Log(USERLOG,"QueryTopics: %s %d ",word,FACTSET_COUNT(store));
...
static bool AddWord2Scan(int flags,MEANING M,MEANING from,int depth,unsigned int type,int fieldid) // mark (and maybe queue) this word + implied wordnet up hierarchy + auto-equivalences
{
	...
	if (flags & QUEUE) scanQueue[queueIndex++] = M; // our word is now on the q
	...
	if (trace & TRACE_QUERY && CheckTopicTrace("^query"))
	{
		...
		Log(USERLOG,(flags & QUEUE) ? (char*)" %s+" : " %s. ",WriteMeaning(M));
	}
...
static bool AddWordOnly(int flags,char* word,unsigned int type,int fieldid) // mark (and maybe queue) this word 
{
	...
	D->inferMark = saveMark; 
	if (flags & QUEUE) scanQueue[queueIndex++] = MakeMeaning(D);

	if (trace & TRACE_QUERY && CheckTopicTrace("^query"))  Log(USERLOG,(flags & QUEUE) ? (char*)" %s+" : (char*)" %s. ",D->word);
...
{// use two fields to select a third. Then look at facts of that third to find a matching verb.
	...
	if (trace & TRACE_QUERY && CheckTopicTrace("^query"))
	{
		...
		if (baseFlags & USE_ORIGINAL_SUBJECT) Log(USERLOG,"Riccochet incoming (%s %s %s) via subject %s using %s\r\n",S->word,V->word,O->word,D1->word,use);
		else Log(USERLOG,"Riccochet incoming (%s %s %s) via object %s using %s\r\n",S->word,V->word,O->word,D1->word,use);
	}
...
static bool ConceptPropogateTest(MEANING M,unsigned int mark,unsigned int depth)  // is this meaning ultimately in a marked set
{
	...
	while (F)
	{
		if (F->verb == Mmember && F->subject == M)
		{
			...
			if (trace & TRACE_QUERY && CheckTopicTrace("^query"))  Log(USERLOG," %d->%s ",depth,D->word);
...
unsigned int Query(char* kind, char* subjectword, char* verbword, char* objectword, unsigned int count, char* fromset, char* toset, char* propogate, char* match)
{
	...
	if (trace & TRACE_QUERY ) 
	{
		...
		if (*fromset != '?') Log(USERLOG,"fromset:%s ",fromset);   
		if (*toset != '?') Log(USERLOG,"toset:%s ",toset);   
		if (*propogate != '?') Log(USERLOG,"propogate:%s ",propogate);   
		if (*match != '?') Log(USERLOG,"match:%s ",match);   
		Log(USERLOG,"\r\n");   
		Log(USERLOG,"");
	}
	...
	while (*++control && *control != ':' )
	{
		...
		switch(*control) // decompose what to do 
		{
			...
			case 'n':  // ignore all member facts involving topic as object
				if (trace & TRACE_QUERY && CheckTopicTrace("^query")) Log(USERLOG," ignore all member facts w topics as objects ");
				++control; 
				baseFlags |= NOTOPIC;
				continue;
			...
			case 'i': 
				++control;
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," ignore #%c results ", *control);
		...
		if (choice) // if we have something to follow, do the requested thing to it
		{
			...
			else if (choice[0] == USERVAR_PREFIX && choice[1]) choice = GetUserVariable(choice, false);
			...
			else if (choice[0] == '@' && GetSetID(choice) != ILLEGAL_FACTSET )
			{
				...
				{
					Log(USERLOG,"\r\n");
					Log(USERLOG,"FactField: %c(%d) ",saveMark-baseMark+'0',saveMark);
				}
				choice = NULL;
				for (unsigned int j = 1; j <= FACTSET_COUNT(whichset); ++j) 
				{
					...
					{
						scanQueue[queueIndex++] = Fact2Index(F);
						continue;
					}
			...
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," raw ");
				++choice;
			}
			...
			{ //   callArgumentList are:  flowverbs,  queue or mark, 
				...
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query"))
				{
					if (flag) Log(USERLOG," Tag+Queue Propogate %c ",kind);
					else Log(USERLOG," Tag Propogate %c ",kind);
				}
		...
		else if (baseFlags & USERFACTS) // transfer over user flags
		{
			...
			while (++F <= lastFactUsed) scanQueue[queueIndex++] = Fact2Index(F);
			baseFlags |= FACTTYPE; 
		}
	}
...
*/
	...
	if (*control && --control) while (*++control && *control != ':' )
	{
		...
		case '!': //   do NOT match this
			...
			if (*control == 's')  
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," don't match subjects #%c ",*control);
				markns = baseMark + (*++control - '0');
			}
			if (*control == 'v')  
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," don't match verbs #%c ",*control);
				marknv = baseMark + (*++control - '0');
			}
			if (*control == 'o')  
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," don't match objects #%c ",*control);
				markno = baseMark + (*++control - '0');
			}
			if (*control == 'n')
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," don't match concepts with noconceptlist marking ");
				noSystemFlag = true;
		...
		case 's': 
			...
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," subject must be #%c ",*control);
			break;
		case 'v': 
			markv = baseMark + (*++control - '0'); 
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," verb must be #%c ",*control);
			break;
		case 'o': 
			marko = baseMark + (*++control - '0'); 
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," object must be #%c ",*control);
			break;
			
		//   dont pay attention to this value during search (opposite the baseOffset)
		case 'i': 
			++control;
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," ignore results with #%c ",*control);
			ignoremark = (*control == '0') ? 0 : (baseMark + (*control - '0'));
			break;
		...
		case '<':  case '>': 
			...
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," propogate on verb #%c ",*control);
			propogateVerb = baseMark + (*++control - '0'); //   label of verbs to propogate on
			break;
		case '@': //   where to put answers (default is store)
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG," store facts in @%c",*control);
			whichset = *++control - '0';
			break;
		case '^':
...
*/
	...
	if (*control && --control) while (*++control && *control != '|')
	{
		...
		case 'S': 
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG,"Riccochet on Subject #%c ",*control);
			rmarks = baseMark + (*++control - '0'); 
			break;
		case 'V': 
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG,"Riccochet on Verb #%c ",*control);
			rmarkv = baseMark + (*++control - '0'); 
			break;
		case 'O': 
			if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG,"Riccochet on Object #%c ",*control);
	...
	if (trace & TRACE_QUERY  && CheckTopicTrace("^query"))
	{
		Log(USERLOG,"Start processing loop\r\n");
		Log(USERLOG,"");
	}
	...
	{
		MEANING next = scanQueue[scanIndex++];
        unsigned int index;
		...
		while (F)
		{
			...
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG,"ignore ");
				continue;
			}
			// pay no attention to topic facts
			if (baseFlags & NOTOPIC && O->internalBits & TOPIC)
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace("^query")) Log(USERLOG,"notopic ");
				continue;
			}
